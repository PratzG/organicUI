 ...
 */
export const executeAction = (components, action) => {
    // Route the action to the appropriate handler based on action type
    switch (action.type) {
        case 'add':
            return addComponent(components, action.payload);
        case 'remove':
            return removeComponent(components, action.payload);
        case 'place':
            return placeComponent(components, action.payload);
        case 'reply':
