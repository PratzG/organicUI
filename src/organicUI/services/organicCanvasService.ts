 ...
 */
const getComponentName = (component: ComponentInstance): string => {
...
export const getCanvasInfo = (
  ...
  const occupiedCells = placedComponents.filter(
    c => c.position.row <= rows && c.position.col <= cols
  ).length;
...
export const getOccupiedCells = (
  ...
  const occupiedCells: GridCellInfo[] = [];
  const canvasInfo = getCanvasInfo(rows, cols, components);
 ...
 */
export const validateCanvasParameters = (rows: number, cols: number): {
  isValid: boolean;
  error?: string;
} => {
  if (rows < 1 || rows > 10) {
 ...
 */
export const updateCanvasParameters = (
  newRows: number,
  newCols: number,
  components: ComponentInstance[]
): {
  ...
  const movedComponents: ComponentInstance[] = [];
  const updatedComponents = components.map(component => {
    if (
      component.position.row > newRows ||
